...
export class BundledDev {
  ...
  /**
   * Facade over the engine's module graph, installed as the environment's
   * `moduleGraph` (see the DevEnvironment constructor). All
   * `EnvironmentModuleNode` construction under bundled dev happens in here.
   */
  ...
  /**
   * `moduleGraph.invalidateAll()` from a plugin maps to a full rebuild plus a
   * page reload (the reload is deferred until the rebuild lands in
   * `onOutput`, like every other server-sent reload).
   */
